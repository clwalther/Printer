...
class Inner_Support_Structure():
    ...
    def getPlanesAlgonX(self):
        ...
        while xValue <= self.maxPoint[0] / 2:
            self.planes[0].append(self.utils.getPlaneX(self.maxPoint[0] / 2 + xValue))
            self.planes[0].append(self.utils.getPlaneX(self.maxPoint[0] / 2 - xValue))
    ...
    def getPlanesAlgonY(self):
        ...
        while yValue <= self.maxPoint[1] / 2:
            self.planes[1].append(self.utils.getPlaneY(self.maxPoint[1] / 2 + yValue))
            self.planes[1].append(self.utils.getPlaneY(self.maxPoint[1] / 2 - yValue))
    ...
    def getPolygons(self):
        for orientation in range(2):
            for plane in self.planes[orientation]:
                faces   = self.utils.getCloseFaces(plane, orientation, self.VERTICES, self.FACES)
                vectors = self.utils.getIntersectingVectors(faces, plane)
                polygon = self.utils.generatePathway(vectors)
...
class Outer_Support_Structure():
    ...
    def setOuterSamplePoints(self):
        for root in self.rootPoints:
            samplePoints = self.utils.getHexPoints(root[0], root[1], 0, self.OUTER_DENSITY)
    ...
    def isBeneathObj(self, point):
        for face in self.FACES:
            ...
            isInside = self.utils.pointInPolygon(polygon, point)
            if isInside:
                return True
        return False
    ...
    def getHeights(self, point):
        ...
        for face in self.FACES:
            polygon   = self.utils.constructPolygon(face, self.VERTICES)
            isBeneath = self.utils.pointInPolygon(polygon, point)
            if isBeneath:
                line     = self.utils.getLineFromRoot(point, X=0, Y=0, Z=1)
                inter, _ = self.utils.getPointLineIntersectPlane(line, polygon)
    ...
    def generateSupportPillar(self, lowerPoint, upperPoint):
        ...
        upperRing = self.utils.getHexPoints(lowerPoint[0], lowerPoint[1], upperRingZ, pillarRadius)
        lowerRing = self.utils.getHexPoints(lowerPoint[0], lowerPoint[1], lowerRingZ, pillarRadius)
